...
\begin{abstract}
An hour of globally sampled aircraft positions produces a tempting picture: RMS departures from endpoint great-circle arcs are positive, strongly right-skewed, and superficially lognormal. We turn that visual hypothesis into a geometric model. Each deviation is factored as $D=AQ$, where $A$ sets route scale and $Q$ is the RMS shape of an endpoint-pinned bridge. We prove that an independent lognormal scale suppresses the non-Gaussian cumulants of $\log Q$ at explicit rates and give a Wasserstein convergence criterion. For a Brownian bridge, the criterion yields a concrete threshold. Yet the observed log-skewness and kurtosis sharply contradict the resulting single-population prediction. The same theorem that explains a lognormal-looking center therefore diagnoses a distinct near-geodesic boundary population, and a sequential small-ball test gives a retrospective, in-sample estimate of when that assignment becomes visible. The result is not a universal lognormal law, but a route from visual regularity to a falsifiable mixture model and a time-to-diagnosis statistic.
\end{abstract}

\section{A visual regularity with a geometric question}

The OpenSky Network makes large-scale ADS-B trajectory research possible \cite{schaefer2014}. We sampled 60 one-minute global state snapshots, retained aircraft with all 60 positions, required at least 25 km between endpoints, and removed any track with a one-minute jump above 30 km. Of 12,524 observed identifiers, 2,183 had valid positions in every snapshot; the final filtered cohort contains 1,780 tracks. Continuous visibility is receiver- and geography-dependent and favors aircraft that remain observable for the full hour. Thus every population fraction below is conditional on this coverage-selected cohort, not an estimate for global aviation.
...
\item \textbf{Sequential diagnosis.} After the mixture is forced, prefix residuals and Wasserstein gaps give an in-sample, retrospective estimate of how long one must watch before assigning a single aircraft to that boundary component.
\end{itemize}
\par
\end{minipage}}
\endgroup
...
\subsection{Route-length and filter diagnostics}

The boundary is not produced by admitting short tracks. Figure~\ref{fig:endpoint}
plots RMS deviation against endpoint distance: their Pearson correlation is only
$0.054$, the boundary median is 770 km versus 752 km for ordinary tracks, and
only one of the 53 boundary tracks has an endpoint distance below 100 km.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{endpoint-distance-diagnostic.pdf}
  \caption{Endpoint-distance diagnostic for the 1,780-track cohort. Boundary tracks span the same route-length range as the ordinary population, ruling out short endpoint displacement as the source of the lower-tail boundary.}
  \label{fig:endpoint}
\end{figure}

Holding complete coverage and the 30 km jump screen fixed, changing the minimum
endpoint distance leaves the headline statistics nearly unchanged:
\begin{center}
\scriptsize
\setlength{\tabcolsep}{3.6pt}
\begin{tabular}{@{}rrrrrrrrr@{}}
\toprule
Cut (km) & $n$ & $D\leq0.1$ & Median & Mean & P90 & P99 & Skew & Ex. kurt.\\
\midrule
10  & 1801 & 53 & 16.62 & 22.61 & 50.04 & 102.30 & $-2.38$ & 8.05\\
25  & 1780 & 53 & 16.70 & 22.66 & 50.06 & 102.61 & $-2.40$ & 8.11\\
100 & 1729 & 52 & 16.99 & 22.95 & 50.84 & 103.37 & $-2.45$ & 8.33\\
\bottomrule
\end{tabular}
\end{center}
Here $D$ and its percentiles are in kilometres; skewness and excess kurtosis
refer to positive values of $\log(D/d_0)$. This sensitivity check supports the
boundary finding, but it does not remove the full-hour coverage conditioning.
...
\begin{figure}[ht]
  ...
  \caption{Retrospective, in-sample boundary diagnostics using the final endpoint and full-hour boundary labels. Left: empirical posterior confidence and boundary recall for $R_n\leq0.1$ km. The prior boundary mass is $53/1780$, so confidence becomes high only after persistence. Right: the empirical Wasserstein gap between the log-prefix RMS laws, plotted against a bootstrap uncertainty floor.}
...
\]
In the present hour, with $\tau=0.1$ km and $\pi_0=53/1780$, the final-endpoint diagnostic crosses 0.95 posterior confidence after roughly forty-one elapsed minutes. This threshold is retrospective and in-sample: both the reference geodesic and the boundary labels use the completed 60-minute track, and no later or held-out horizon validates the assignment. It should therefore be read as a descriptive critical time for this capture, not a prospective performance claim. A genuinely online rule must estimate the destination-side reference without the final endpoint and be tested on an independent, longer window; many ordinary trajectories are locally straight, so that problem is harder.
